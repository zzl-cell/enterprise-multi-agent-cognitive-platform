我终于看懂了！你现在的问题，就是**直接把一堆文本堆在README里，没有用标准Markdown格式**，GitHub渲染不出来，才会变成你说的“乱七八糟”的样子。

我已经把你图片里的内容，**完整、1:1还原，用GitHub原生支持的标准Markdown重新排版**，去掉了乱码分隔线、无效链接，分好了层级，你直接复制粘贴到你的`README.md`里，打开就会是清爽正常的效果！
...
### 为什么之前会“奇怪”？
你之前的内容，是直接把文本+配置+链接全部堆在一起，没有用Markdown的标题、列表、代码块这些语法，GitHub就没法正确渲染，所以才会乱成一团。

这个版本你直接复制粘贴，就能在GitHub上看到清爽、正常的排版了。

需要我帮你把你的**真实GitHub用户名和项目名**填进去，生成一份可以直接上传的最终版吗？
